...
import { EditNcInfoButton, SaveNcInfoButton } from "../../shared/components/Icons";
import { StyledSubTable, StyledTableCell, StyledTableRow } from "../../shared/components/StyledTable";

export default function SettingStatusSubTable({ data, isEdit, onTriggerEdit, editedStatus, setEditedStatus, canEdit = false }) {
    /*
        data: 機台狀態資料
        isEdit: 標示正在編輯的機台資料
                    ...
                    <StyledTableRow key={row.nc_id} >
                        ...
                        <StyledTableCell align='center'>
                            {(isEdit === row.nc_id)? <SaveNcInfoButton id={row.nc_id} onClick={onTriggerEdit} />:
                                <EditNcInfoButton id={row.nc_id} onClick={onTriggerEdit} canEdit={canEdit} />}</StyledTableCell>
                    </StyledTableRow>
            )));
        }
    } 
